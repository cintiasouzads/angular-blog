 export const dataFake = [
  {
    "id":"1",
    "title":"",
    "description":"",
    "photoCover":"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ9tJeT7NdcZvjsqaZ-fkKTOjkZCw60hX_m6A&usqp=CAU"

  },
  {
  "id":"2",
   "title":"",
    "description":"A empatia e a diplomacia podem ser bem presentes na personalidade de quem tem o sol no signo Libra. Naturalmente direcionados para as relações com outras pessoas, librianas e librianos podem ter um grande talento para solucionar conflitos, porém, a preocupação em racionalizar os dois lados de uma questão pode gerar situações de dúvida e indecisão.Libra é regido por Vênus, e isso dá ao signo um apreço natural a tudo que pode ser considerado belo. Quem tem um acúmulo de planetas em Libra pode ter uma forte conexão com as artes, ou até ter facilidade para viver uma carreira artística. Em casos com pouca maturidade, essa aproximação com a beleza pode se manifestar como uma vaidade desmedida. O companheirismo pode ser uma característica marcante, como também o senso de justiça e a busca em viver todas as relações com respeito mútuo.",
    "photoCover":"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR3YmWN1NZZafHoTJ4vPz2kCoarAtyw1HkhWg&usqp=CAU"
  },
  {
    "id":"3",
    "title":"",
    "description":"Libra é praticamente sinônimo de relacionamento, pois é o signo ligado às relações e parcerias. Relacionamento é seu objetivo de vida, por mais que a libriana possa negar em sua fase solteira. Um namoro ou belo casamento são objetos de desejo e de realização. Não precisa ser nada formal, basta o estar junto curtindo momentos bem instagramáveis e românticos. Se a libriana está num namoro, sua autoestima fica lá em cima, e ela fica até mais ousada para experimentar umas aventuras diferentes.",
    "photoCover":"https://ellebrasil-wp-images.s3.amazonaws.com/2022/08/origin-152.png"
  },
  {
    "id":"4",
    "title":"",
    "description":
    "Libra e Áries. Áries é o oposto complementar de Libra e, por isso, traz tudo o que falta no libriano: atitude, vigor, disposição para brigar pelo que é necessário, garra e dinamismo. Essa relação vai pegar fogo e trazer movimento para a vida sempre suave do nativo de Libra.  No entanto, as atitudes impulsivas de Áries podem desiquilibrar a pessoa de Libra, que precisará  exercitar a paciência. Se conseguirem superar essas dificuldades, poderão viver uma boa relação.",
    "photoCover": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS52y2zCqcjSMOHuCayww548V7F_wWVAoWfnQ&usqp=CAU"
  }
 ]



